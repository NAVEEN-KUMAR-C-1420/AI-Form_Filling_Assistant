...
class OCRService:
    ...
    ENTITY_PATTERNS = {
        ...
        EntityType.PAN_NUMBER: [
            ...
            r'(?:Permanent Account Number|PAN|P\.A\.N)[:\s]*([A-Z]{5}[0-9]{4}[A-Z])\b',
            r'(?:Account Number)[:\s]*([A-Z]{5}[0-9]{4}[A-Z])\b',
            ...
            # Very flexible pattern - some letters may be misread (e.g., AZHPN8387P could be AZHPNB3B7P)
        ...
        EntityType.DATE_OF_BIRTH: [
            # DOB label patterns with various separators
            r'(?:DOB|D\.O\.B|Date of Birth|जन्म तिथि|பிறந்த தேதி)[:\s]*[:\s]*(\d{2}[/\-\.]\d{2}[/\-\.]\d{4})',
            # Year of Birth patterns (some Aadhaar cards only have year)
            r'(?:Year of Birth|YOB|वर्ष|பிறந்த ஆண்டு)[:\s]*[:\s]*(\d{4})',
    ...
    def _extract_name(self, text: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        # Fallback: For Aadhaar/PAN cards, try to find names by looking for lines that are all letters
        lines = text.split('\n')
        ...
        for i, line in enumerate(lines):
            ...
            if clean_line and re.match(r'^[A-Za-z\s\.]+$', clean_line):
                ...
                # Valid names typically have proper vowel/consonant patterns
                if not self._is_valid_english_name(clean_line):
                    continue
                
                # Skip S/O, D/O, W/O lines - those are father/husband references
                if re.match(r'^[SDWC]/O\s', clean_line, re.IGNORECASE):
                    continue
                    
                # Skip if too short or too long
                if 3 <= len(clean_line) <= 50:
                    name = re.sub(r'\s+', ' ', clean_line).title()
                    # If we found "To" label, this is likely the name (Aadhaar format)
                    confidence = 0.85 if found_to_label else 0.75
                    logger.info(f"Found potential name from line scan: {name} (after_to={found_to_label})")
                    return {
                        "entity_type": EntityType.FULL_NAME.value,
                        "value": name,
                        "original_language": language,
